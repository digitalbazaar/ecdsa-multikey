...
import {KeyPairSecp256k1} from './KeyPairSecp256k1.js';
...
const PKCS8_PREFIXES = new Map([
  ...
  [ECDSA_CURVE.secp256k1, {
    secret: new Uint8Array([
      ...
      // uncompressed index 2 value 132
      // not sure if it can support compressed at all (length = 103?)
...
function decompressKey(pubkey) { // pubkey should be 33 bytes
  return Uint8Array.from(
    KeyPairSecp256k1.keyFromPublic(pubkey, 'bytes').getPublic(false, 'array')
  );
}
