...
async function _createKeyPairInterface({keyPair, keyAgreement = false}) {
  // don't do import keyPair when curve is secp256k1,
  // as polyfilled it won't be understood by importKeyPair
  if(!(keyPair?.publicKey instanceof CryptoKey) &&
    keyPair.publicKey?.algorithm.namedCurve !== ECDSA_CURVE.secp256k1) {
    keyPair = await importKeyPair(keyPair);
  }
