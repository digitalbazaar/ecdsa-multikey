...
export async function generate({
  ...
  if(curve === ECDSA_CURVE.secp256k1) {
    ...
    keyPair = await cryptoPolyfill.subtle.generateKey(
...
export async function toJwk({keyPair, secretKey = false} = {}) {
  ...
  let jwk;
  if (keyPair.publicKey.algorithm.namedCurve === ECDSA_CURVE.secp256k1) {
    const {Crypto} = await import('@peculiar/webcrypto');
    const cryptoPolyfill = new Crypto();
...
async function _createKeyPairInterface({keyPair, keyAgreement = false}) {
  // don't do import keyPair when curve is secp256k1, as polyfilled it won't be understood by importKeyPair
  if(!(keyPair?.publicKey instanceof CryptoKey) && keyPair.publicKey?.algorithm.namedCurve !== ECDSA_CURVE.secp256k1) {
    keyPair = await importKeyPair(keyPair);
  }
...
async function _deriveSecret({localKeyPair, remoteKeyPair}) {
  ...
  if(curve === ECDSA_CURVE.secp256k1) {
    const {Crypto} = await import('@peculiar/webcrypto');
    const cryptoPolyfill = new Crypto();
    const arrayBuffer = await cryptoPolyfill.subtle.deriveBits({
      name: 'ECDH',
      namedCurve: curve,
